...
// –јЅќ“ј — “≈√јћ». „ј—“№ 1
	...
	if( $botParams->get( 'shortatend' ) ) $text = preg_replace( '/(?<=[а-€ј-яa-zA-Z]) ([а-€ј-яa-zA-Z]{1,5}(?>[.!?Е]*))(?=$|<\/p>|<\/div>|<br>|<br \/>)/',NOBRSPACE.'\\1', $text); // ѕоследнее короткое слово в абзаце прив€зывать к предыдущему

//ѕ–яћјя –≈„№
	if( $botParams->get( 'speech' ) ) $text = preg_replace( '/(?<=^|<p>|<br>|<br \/>)[ †]?- /','Ч†', $text ); // ѕр€ма€ речь - дефис в начале строки и после тегов <p>, <br> и <br />
...
//	$text = str_replace( 'Х ','Х'.NOBRSPACE, $text ); // ѕункт (дл€ списков)
	...
	if( $botParams->get( 'plusminus' ) ) { // плюс-минус
	...
	if( $botParams->get( 'copyright' ) ) $text = preg_replace( '/\(\s?(с|—|c|C)\s?\)/','©', $text );
...
// ƒ–ќЅ»
	...
	{
		$text = preg_replace( '/(?<=^|[ †("ЂД])1\/2(?=$|[ †)"їУ.,!?:;Е])/','&frac12;', $text);
		$text = preg_replace( '/(?<=^|[ †("ЂД])1\/4(?=$|[ †)"їУ.,!?:;Е])/','&frac14;', $text);
		$text = preg_replace( '/(?<=^|[ †("ЂД])3\/4(?=$|[ †)"їУ.,!?:;Е])/','&frac34;', $text);
	}
	if( $botParams->get( 'fracext' ) ) 
	{
		$text = preg_replace( '/(?<=^|[ †("ЂД])1\/3(?=$|[ †)"їУ.,!?:;Е])/','&#8531;', $text);
		$text = preg_replace( '/(?<=^|[ †("ЂД])2\/3(?=$|[ †)"їУ.,!?:;Е])/','&#8532;', $text);
		$text = preg_replace( '/(?<=^|[ †("ЂД])1\/8(?=$|[ †)"їУ.,!?:;Е])/','&#8539;', $text);
		$text = preg_replace( '/(?<=^|[ †("ЂД])3\/8(?=$|[ †)"їУ.,!?:;Е])/','&#8540;', $text);
		$text = preg_replace( '/(?<=^|[ †("ЂД])5\/8(?=$|[ †)"їУ.,!?:;Е])/','&#8541;', $text);
		$text = preg_replace( '/(?<=^|[ †("ЂД])7\/8(?=$|[ †)"їУ.,!?:;Е])/','&#8542;', $text);
	}
...
// —ќ –јў≈Ќ»я
	...
	{
		$text = preg_replace( '/(?<=[^а-€ј-яЄ®a-zA-Z]|^)(г\.|ул\.|пер\.|пл\.|пос\.|р\.|проф\.|доц\.|акад\.|гр\.) ?(?=[ј-я®])/', '\\1'.NOBRSPACE, $text ); // —окращени€
		$text = preg_replace( '/(?<=[^а-€ј-яЄ®a-zA-Z]|^)(с\.|стр\.|рис\.|гл\.|илл\.|табл\.|кв\.|дом|д.\|офис|оф\.|ауд\.) ?(?=\d)/', '\\1'.NOBRSPACE, $text ); // —окращени€
		$text = preg_replace( '/(?<=[^а-€ј-яЄ®a-zA-Z]|^)(см\.|им\.|каф\.) ?(?=[а-€ј-яЄ®a-zA-Z\d])/', '\\1'.NOBRSPACE, $text ); // —окращени€
	}
	
// ≈ƒ»Ќ»÷џ »«ћ≈–≈Ќ»я
	if( $botParams->get( 'units' ) )
	{
		$text = preg_replace( '/(?<=[а-€Єa-z\d\.]) (?=экз\.|тыс\.|млн\.|млрд\.|руб\.|коп\.|у\.е\.|\$|И)/', NOBRSPACE, $text ); // ≈диницы измерени€
		$text = preg_replace( '/(?<=[а-€Єa-z\d\.]) (?=евро([ \.,!\?:;]|$))/', NOBRSPACE, $text ); // ≈вро
	}

// ѕ–»¬я«џ¬ј≈ћ  ќ–ќ“ »≈ —Ћќ¬ј
	if( $botParams->get( 'shortwords' ) )
	{
		$text = preg_replace( '/(?<=[^а-€ј-яЄ®a-zA-Z0-9])(€|ты|мы|вы|он|не|ни|на|но|в|во|до|от|и|а|с|со|о|об|ну|к|ко|за|их|из|ее|еЄ|ей|ой|ай|у) (?=[а-€ј-яЄ®a-zA-Z]{3})/', '\\1'.NOBRSPACE, $text ); //  ороткие слова прикрепл€ем к следующим (если те сами не короткие)
		$text = preg_replace( '/(?<=[а-€ј-яЄ®]) (?=(же|ж|ли|ль|бы|б|ка|то)([\.,!\?:;])?( |$))/', NOBRSPACE, $text ); // „астицы
...
// » “.ƒ., » “.ѕ., “. ., ...
	...
	{
		$text = preg_replace( '/(?<=[^а-€ј-яЄ®a-zA-Z]и) (д|п)(?=р\.)/', NOBRSPACE.'\\1', $text ); // и др., и пр.
		$text = preg_replace( '/(?<=[^а-€ј-яЄ®a-zA-Z]и) т\. ?(?=(д|п)\.)/', NOBRSPACE.'т.'.THINSP, $text ); // и т.д., и т.п.
		$text = preg_replace( '/(?<=[^а-€ј-яЄ®a-zA-Z]в) т\. ?(?=ч\.)/', NOBRSPACE.'т.'.THINSP, $text ); // в т.ч.
		$text = preg_replace( '/(?<=[^а-€ј-яЄ®a-zA-Z]т\.) ?(?=(к|н|е)\.)/', THINSP, $text ); // т.к., т.н., т.е.
		$text = preg_replace( '/(?<=[^а-€ј-яЄ®a-zA-Z](к|д)\.) ?(ф\.-м|х|б|т|ф|п)\. ?(?=н\.)/', THINSP.'\\1.'.THINSP, $text ); // к.т.н., д.ф.-м.н., ...
	}

// »—ѕ–ј¬Ћ≈Ќ»≈ √–јћћј“»„≈— »’ ќЎ»Ѕќ 
	if( $botParams->get( 'spinquotes' ) ) $text = preg_replace( '/\( *([^)]+?) *\)/', '(\\1)', $text ); // удал€ем пробелы после открывающей скобки и перед закрыващей скобкой
	if( $botParams->get( 'spbeforequote' ) ) $text = preg_replace( '/(?<=[а-€ј-яЄ®a-zA-Z.,!?:;Е])\(/', ' (', $text ); // добавл€ем пробел между словом и открывающей скобкой, если его нет (отключите, если у ¬ас на сайте есть формулы)
	if( $botParams->get( 'commanum' ) ) $text = preg_replace( '/(?<=[а-€ј-яЄ®a-zA-Z]),(?=\d)/',', ', $text); // ƒелает проверку в расстановке зап€тых и мен€ет слово,число на слово, число (ул. ƒружбы, 46)
	...
	if( $botParams->get( 'exclquestdots' ) ) $text = preg_replace( '/(?<=!|\?)(?:Е|\.\.\.)/','..', $text ); // ”бираем лишние точки
	if( $botParams->get( 'sppunct' ) ) $text = preg_replace( '/ (?=[.,!?:;])/','', $text ); // ”бираем пробелы перед знаками препинани€
	if( $botParams->get( 'nummarksp' ) ) $text = preg_replace( '/(?<=є|І) ?(?=\d)/', '†', $text ); // пробел между знаком "є" или "І" и числом.
...
// ¬—® ќ „»—Ћј’
	if( $botParams->get( 'sizes' ) ) $text = preg_replace( '/(?<=\d) *(?:\*|х|x|X|’) *(?=\d)/', '&times;', $text ); // обрабатываем размерные конструкции (200x500)
	...
	if( $botParams->get( 'numnbsp' ) ) $text = preg_replace( '/(?<=[ '.DASH.NUMDASH.']|^)((?>\d+)) /','\\1'.NOBRSPACE, $text ); // Ќеразрывный пробел после арабских цифр
	if( $botParams->get( 'romenbsp' ) ) $text = preg_replace( '/(?<=[ '.DASH.NUMDASH.']|^)((?>[IVXLCDM]+)) /','\\1'.NOBRSPACE, $text ); // Ќеразрывный пробел после римских цифр
	...
	if( $botParams->get( 'percent' ) ) $text = preg_replace( '/(?<=\d)[ †'.NOBRSPACE.'](?=%|Й)/','', $text); // «наки процента (%) и промилле (Й) прикрепл€ть к числам, к которым они относ€тс€
	if( $botParams->get( 'numnum' ) ) $text = preg_replace( '/(?<=\d) (?=\d)/',NOBRSPACE, $text ); // Ќе разрывать 25 000

// –ј«Ќќ≈
	if( $botParams->get( 'orgs' ) ) $text = preg_replace( '/(?<=ќќќ|ќјќ|«јќ|„ѕ) ?(?="|Ђ)/',NOBRSPACE, $text); // ƒелает неразрывными названи€ организаций и абревиатуру формы собственности
...
// “»–≈ ¬ ƒ»јѕј«ќЌ≈
	...
	{
	case 0:
		$text = str_replace( NUMDASH,'Ч', $text ); // mdash
	case 2:
	...
	case 1:
	default:
		$text = str_replace( NUMDASH,'Ц', $text ); break; // ndash
	}
...
// –јЅќ“ј — “≈√јћ». „ј—“№ 2
	...
	//”дал€ем пустые параграфы
	if( $botParams->get( 'delEmptyParagraph' ) ) {
		$text = preg_replace( '/<p[^>]*>(&nbsp;| |<br[^>]*>)*<\/p>/','', $text); // ”дал€ем пустые <p></p>
	};
