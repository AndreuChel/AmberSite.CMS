...
function get_component(name) {
    if (this.Components[name])
        return this.Components[name]['link'];
    else
        return false;
}
//********************************************************************************************
...
function ArrayToParams(params_array) {
    ...
    for (i in params_array) {
        if (params_array[i])
            params_array[i] = params_array[i].toString();
        if (!flag_resh) {
...
function XPathRes(xml_obj, quary) {
    ...
    if (this.Server.brouzer == "IE")
        return xml_obj.documentElement.selectSingleNode(quary);
    else
        return xml_obj.evaluate(quary, xml_obj, null, XPathResult.ANY_TYPE, null).iterateNext();
}
//********************************************************************************************
function SetInnerText(_node, _val) {
    _val = this.unhtml_special(_val);
    try {
        _node.firstChild.nodeValue = _val;
    } catch (ex) {
        if (this.Server.brouzer == "IE") {
            var tmp_parser = new ActiveXObject("Msxml2.DOMDocument");
            var text_node = tmp_parser.createTextNode(_val);
        } else
            var text_node = document.createTextNode(_val);
